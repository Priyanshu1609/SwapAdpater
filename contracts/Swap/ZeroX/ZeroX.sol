...
/**
 * @title ZeroXUniswapV3
 * @notice Swapper contract for 1inch UniswapV3 swaps.
 */
contract ZeroXSwapper is ISwapper {
  using Address for address;

  IUniswapV3Router public immutable ZeroXUniRouter;

  constructor(address _ZeroXUniRouter) {
    ZeroXUniRouter = IUniswapV3Router(_ZeroXUniRouter);
  }
  ...
  function swap(
    ...
    if (_fromAsset != _toAsset) {
      ...
      if (IERC20(_fromAsset).allowance(address(this), address(ZeroXUniRouter)) < _amountIn) {
        TransferHelper.safeApprove(_fromAsset, address(ZeroXUniRouter), type(uint256).max);
      }

      // The call to `uniswapV3Swap` executes the swap.
      // use actual amountIn that was sent to the xReceiver
      amountOut = ZeroXUniRouter.uniswapV3Swap(_amountIn, _minReturn, _pools);
    } else {
      amountOut = _amountIn;
    }
  ...
  function swapETH(
    ...
    require(msg.value >= _amountIn, "ZeroXUniswapV3: msg.value != _amountIn");
